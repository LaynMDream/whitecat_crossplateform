...
int key_presetvideo(int preset)
{
    ...
    {
        tracking_dock_selected=preset;   //les banques de tracking cam
    }
    return(0);
}
    ...
    case 0:
        ...
        if(index_blind==0)
        {
            for (int ci=1; ci<514; ci++)
            {
                ...
                {
                    if(bufferSequenciel[ci]>0 && Selected_Channel[ci]==1 )
                    {
                        Selected_Channel[ci]=0;
                    }
                    else if (bufferSequenciel[ci]>0 && Selected_Channel[ci]==0)
                    {
...
int key_select_all()
{
    ...
    else
    {
        if(index_blind==0)
        {
            for (int ci=1; ci<514; ci++)
            {
                if(bufferSequenciel[ci]==0)
                {
                    Selected_Channel[ci]=0;
                }
                else if (bufferSequenciel[ci]>0)
                {
                    Selected_Channel[ci]=1;
                }
            }
