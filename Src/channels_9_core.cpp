...
int Channel_select_inv()
{
    if(index_blind==0)
    {
        for (int ci=1; ci<514; ci++)
        {
            if(bufferSaisie[ci]>0 && Selected_Channel[ci]==1)
            {
                Selected_Channel[ci]=0;
            }
            else if (bufferSaisie[ci]>0 && Selected_Channel[ci]==0)
            {
                Selected_Channel[ci]=1;
            }
        }
    ...
    else if(index_blind==1)
    {
        for (int ci=1; ci<514; ci++)
        {
            if(bufferBlind[ci]>0 && Selected_Channel[ci]==1)
            {
                Selected_Channel[ci]=0;
            }
            else if (bufferBlind[ci]>0 && Selected_Channel[ci]==0)
            {
                Selected_Channel[ci]=1;
            }
        }
...
void do_logical_ChannelScroller_wheel()
{
    ...
    if(mousePtr.isOverRecSize(x_left, y_top, 100, 275))
    {
        if (ClassicalChannelView)
        {
            ...
            whc_mouseWheel::c_levelIncrease(whc_mouseWheel::scroll, scroll_channelspace, 245, 1, -5, -20);
            mouseScroll.SetProcessed();
        }
        else
        {
            whc_mouseWheel::c_levelIncrease(whc_mouseWheel::scroll, scroll_channelspace, 275, 1, -5, -20);
            mouseScroll.SetProcessed();
        }
    }
}
