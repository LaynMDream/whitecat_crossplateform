...
int entetes_confirmation()
{
    ...
    else if(index_do_link_membefore==1)
    {
        ...
        {
            sprintf(string_confirmation,"Unlink Mem %d.%d ?", mem_before_one/10,mem_before_one%10);
        }
    }
    ...
    else if(index_do_link_memonstage==1)
    {
        ...
        {
            sprintf(string_confirmation,"Unlink Mem %d.%d ?", position_onstage/10,position_onstage%10);
        }
    }
    else if(index_do_link_memonpreset==1)
    {
        ...
        {
            sprintf(string_confirmation,"Unlink Mem %d.%d ?", position_preset/10,position_preset%10);
        }
    }
    else if(index_do_link_memother==1)
    {
        ...
        {
            sprintf(string_confirmation,"Unlink Mem %d.%d ?", other_mem_in_loop/10,other_mem_in_loop%10);
        }
    }
    ...
    {
        if (couleur_to_affect==0)
        {
            sprintf(string_confirmation,"Affect As RED for Color Preset %d  ?",dock_color_selected+1);
        }
        else if (couleur_to_affect==1)
        {
            sprintf(string_confirmation,"Affect As GREEN for Color Preset %d  ?",dock_color_selected+1);
        }
        else if (couleur_to_affect==2)
        {
            sprintf(string_confirmation,"Affect As BLUE for Color Preset %d  ?",dock_color_selected+1);
        }
        else if (couleur_to_affect==3)
        {
            sprintf(string_confirmation,"Affect As YELLOW for Color Preset %d  ?",dock_color_selected+1);
        }
    }
    ...
    {
//sab 02/03/2014 impact  sprintf(string_confirmation,"Clear completely Chaser %d ?",track_selected_for_record+1,chaser_selected_for_record+1 );
    ...
    else if(index_ask_build_view==1)
    {
        ...
        case 0:
            ...
            if(channel_view_type_of_behaviour[channel_view_is]==1)//mem
            {
                ...
                {
                    sprintf(string_confirmation,"Mem %d.%d doesn't exist ! ",nummem/10,nummem%10);
                }
            }
            else if(channel_view_type_of_behaviour[channel_view_is]==2)//fader
            {
                ...
                {
                    sprintf(string_confirmation,"Fader number must be between 1 and 48 ! ");
                }
            }
            break;
        case 1:
...
int operations_confirmation()
{
    ...
    {
        set_to_default_my_roi(tracking_dock_selected,dock_roi_selected);
        tracker_clearmode=0;
    }
    ...
    else if( index_do_export==1)
    {
        ...
        case 0://ASCII
            ...
            {
                sprintf(string_Last_Order,">>This file is not an ASCII type file (.ASC) !");
            }
            break;
        ...
        case 3://PDF
            ...
            {
                sprintf(string_Last_Order,">>This file is not a pdf type file (.PDF) !");
            }
            break;
        }
    }
    ...
    else if( index_do_import==1)
    {
        ...
        case 1://Schwz
            ...
            {
                sprintf(string_Last_Order,">>This file is not a schwartzpeter FOLDER !");
            }
            break;
        case 2://Alq
            if(isAlq==1)
            ...
            {
                sprintf(string_Last_Order,">>This file is not a Strand ASCII type file (.ALQ) !");
            }
            break;
        }
    }
    ...
    else if(index_do_freeze==1)
    {
        for (int fz=1; fz<514; fz++)
        {
            if(Selected_Channel[fz]==1)
            {
                ...
                {
                    bufferSaisie[fz]=freeze_state[fz];
                }
...
int fenetre_confirm()
{
    ...
    if(mouse_x>XConfirm+110 && mouse_x<XConfirm+110+70 && mouse_y>YConfirm+60 && mouse_y<YConfirm+60+30 && window_focus_id==W_ASKCONFIRM)
    {
        ...
        if(mouse_button==1 && mouse_released==0)
        {
            ...
            //sab 05/03/2014 because add_a_windwow dupplicate protected don't need to substract again
            //add_a_window() substract_a_window(previous_window_focus_id);
            mouse_released=1;
