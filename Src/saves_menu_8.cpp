...
//ligne 1
        Rect SavePin(Vec2D(xrep,yrep+(u*15)),Vec2D(10,10));
        if(specify_who_to_save_load[u]==1)
        {
            SavePin.Draw(CouleurFader);
        }
...
//ligne 2
        ...
        if(specify_who_to_save_load[u+12]==1)
        {
            SavePinSnd.Draw(CouleurFader);
        }
...
//ligne 3
        ...
        if(specify_who_to_save_load[u+24]==1)
        {
            SavePinThrd.Draw(CouleurFader);
        }
...
int Save_Menu(int xsave, int ysave)
{
    ...
    {
        backSave.DrawOutline(CouleurFader);
    }
    ...
    case 1:
        ...
        if(isPdf==1)
        {
            selecteur_PDF_save_solo_global(xsave+30, ysave+50);
        }
...
int do_logical_Save_Menu(int xsave, int ysave)
{
    ...
    case 1:
        if(isPdf==1)
        {
            do_logical_selecteur_PDF_save_solo_global(xsave+30, ysave+50);
        }
