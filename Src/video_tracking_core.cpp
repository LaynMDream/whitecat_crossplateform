...
//premiere ligne les args
    ...
    {
        sprintf(string_ocv_mode,"DIV");
    }
    else if(ocv_calcul_mode==1)
    {
        sprintf(string_ocv_mode,"SUB");
    }
    else if(ocv_calcul_mode==2)
    {
        sprintf(string_ocv_mode,"AbsD");
    }
...
int InitVideo()
{
    ...
    {
        sprintf(string_Last_Order,">>No Camera recognized !");
    }
    return(0);
}
...
//CALCUL
    if(ocv_calcul_mode==0)//MODE BACKGROUND SUPPRESSION
    {
        ...
        if(blur_on==1)
        {
            cvSmooth(onech_difference,onech_difference,CV_BLUR,3,3,0,0);
        }
        if(threshold_on==1)
        {
    ...
    if(pixels_changed>=old_pixels_changed)
    {
        nbre_pixels_changed=pixels_changed-old_pixels_changed;
    }
    else if(pixels_changed<old_pixels_changed)
    {
        nbre_pixels_changed=old_pixels_changed-pixels_changed;
    }
...
//calcul du ratio du tracker
    ...
    if(indicateur_pixels>255)
    {
        indicateur_pixels=255;
    }
    else if(indicateur_pixels<0)
    {
        indicateur_pixels=0;
    }
