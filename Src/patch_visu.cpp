...
//le calcul du SPLINE A GARDER DANS LA FENETRE SINON PLANTAGE///////////////////
    ...
    if( window_focus_id==W_PATCH && index_enable_curve_editing==1 && mouse_x>=XCurv+30 && mouse_x<=XCurv+285 && mouse_y>YCurv+330 && mouse_y<YCurv+355 && mouseClicLeft.isDown())
    {
...
//scroll
    ...
    if(index_affect_patch==1)
    {
        SelectPatchAction.Draw(CouleurFader);
    }
    ...
    {
        SelectPatchActionCurv.Draw(CouleurFader);
    }
    ...
    for (int l=0; l<86; l++)
    {
        if(l==85)
        {
            maxchan_per_ligne=3;   //derniere ligne à 512
        }
