...
int all_at_zero_panel_core(int xf, int yf)
{
    ...
    for (int u=0; u<8; u++)
    {
        ...
        if(mouse_x>xf && mouse_x<xf+50 && mouse_y>yf+10+(25*u) && mouse_y<yf+10+(25*u)+20 && mouseClicLeft.isToBeProcessed())
        {
    ...
    if(mouse_y>ymf && mouse_y<ymf+20 )
    {
        ...
        if(mouse_x>xmf+(24*larg)+10 && mouse_x<xmf+(24*larg)+30)
        {
            ...
            else
            {
                ...
                {
                    position_minifader_selected=core_user_define_nb_faders;
                }
        ...
        else if(mouse_x>xmf+(24*larg)+35 && mouse_x<xmf+(24*larg)+55)
        {
            ...
            {
                position_minifader_selected++;
                if(position_minifader_selected>core_user_define_nb_faders)
                {
...
//select
    if(mouse_x>xmf+(24*larg)+10 && mouse_x<xmf+(24*larg)+55 )
    {
        ...
        if(mouse_y>ymf+25 && mouse_y<ymf+45 && mouseClicLeft.isToBeProcessed() )
        {
            if( Midi_Faders_Affectation_Type!=0)
            {
        ...
        else if(mouse_y>ymf+50 && mouse_y<ymf+70 && mouseClicLeft.isToBeProcessed() )
        {
            if( Midi_Faders_Affectation_Type!=0)
            {
...
////FX MODE
                ...
                if(  window_focus_id==922 && mouseClicLeft.isToBeProcessed() && mouse_x>xmf+(cmptfader*larg) && mouse_x<xmf+(cmptfader*larg)+larg && mouse_y>(ymf+(lfad*hmfd))-12  && mouse_y<(ymf+(lfad*hmfd))-12+5 )
                {
                    if( Midi_Faders_Affectation_Type!=0)
                    {
                    ...
                    else
                    {
                        ...
                        if(fader_mode_with_buffers[cmptfader+(lfad*24)]>5)
                        {
                            fader_mode_with_buffers[cmptfader+(lfad*24)]=0;
                        }
                    }
                    mouseClicLeft.SetProcessed();
                }
...
//store group fader
                    else if(index_do_dock==1 && index_do_fgroup==1 )
                    {
                        ...
                        {
                            if(minifaders_selected[io]==1)
                            {
                                index_there_is_fader_selected=1;
                                break;
                            }
                        }
                        ...
                        {
                            sprintf(string_Last_Order,"No minifaders selected");
                        }
                    }
...
//NIVEAU
                        ...
                        if(Fader[cmptfader+(lfad*24)]>=254)
                        {
                            Fader[cmptfader+(lfad*24)]=255;
                            midi_levels[cmptfader+(lfad*24)]=127;
                        }

                        index_fader_is_manipulated[cmptfader+(lfad*24)]=1;
                        if(midi_send_out[cmptfader+(lfad*24)]==1)
                        {
                            index_send_midi_out[cmptfader+(lfad*24)]=1;
                        }
                        if(lfo_mode_is[cmptfader+(lfad*24)]==1 || lfo_mode_is[cmptfader+(lfad*24)]==2 || lfo_cycle_is_on[cmptfader+(lfad*24)]==1)
                        {
                            lfo_mode_is[cmptfader+(lfad*24)]=0;
                            lfo_mode_is[cmptfader+(lfad*24)]=0;
                            lfo_cycle_is_on[cmptfader+(lfad*24)]=0;
                        }
//CONFIG MIDI
