...
//SPECIFY SIZE
    ...
    if(mouse_x>(x_cfg_sc+100) && mouse_x<(x_cfg_sc+165) && window_focus_id==920)
    {
        if(mouse_y> y_cfg_sc+130 && mouse_y<y_cfg_sc+160)
        {
            ...
            if(mouseClicLeft.isDown() && mouseClicLeft.isToBeProcessed() && index_specify_size==1)
            {
                ...
                {
                    sprintf(string_Last_Order,"Screen Width must be at minimum 1024 px");
                }
                mouseClicLeft.SetProcessed();
            }
        }
        if(mouse_y> y_cfg_sc+165 && mouse_y<y_cfg_sc+195)
        {
            ...
            if(mouseClicLeft.isDown() && mouseClicLeft.isToBeProcessed() && index_specify_size==1)
            {
                ...
                {
                    sprintf(string_Last_Order,"Screen Height must be at minimum 600 px");
                }
                mouseClicLeft.SetProcessed();
            }
        }
...
//set position
    ...
    if(mouse_x>(x_cfg_sc+330) && mouse_x<(x_cfg_sc+395) && window_focus_id==920)
    {
        if(mouse_y> y_cfg_sc+190 && mouse_y<y_cfg_sc+220)
        {
            ...
            if(mouseClicLeft.isDown() && mouseClicLeft.isToBeProcessed() && index_set_pos==1)
            {
...
//////////////////////COLOR THEME/////////////////////////////////////////////
    ...
    for (int thp=0; thp<6; thp++)
    {
        ...
        if(config_color_style_is==thp)
        {
            theme_mode.Draw(CouleurFader);
        }
    }
...
//selection et affectation user
    ...
    if(mouse_x> x_cfg_sc+665 && mouse_x< x_cfg_sc+720 && mouse_y>y_cfg_sc+105 && mouse_y<y_cfg_sc+120 && window_focus_id==920)
    {
        ...
        if(mouseClicLeft.isDown() && mouseClicLeft.isToBeProcessed())
        {
            if( receive_from_trichro==0)
            {
                receive_from_trichro=1;
            }
            else if( receive_from_trichro==1)
            {
    ...
    if(receive_from_trichro==1)
    {
        theme_receive.Draw(CouleurFader);
    }
