...
int draw_pdf_grid(int grd, HPDF_REAL xx, HPDF_REAL yy)
{
    ...
    for(int stp=0; stp<1023; stp++)
    {
        ...
        for(int c=0; c<512; c++)
        {
            if(grid_levels[grd][stp][c]>0)
            {
                steps_to_write[stp]=1;
            }
        }
    }
    ...
    if(specify_who_to_save_PDF[0]==1)
    {
        ...
        for(int m=0; m<10000; m++)
        {
            if(MemoiresExistantes[m]==1)
            {
                ...
                if(MemoiresExclues[m]==1)
                {
                    strcat(header_export, " >>>>> [ MEMORY IS EXCLUDED from CueList ]");
                }
                draw_title(page, 50, debut_lignes -(position_ligne1 +(cmptline_pdf*12)),header_export);
...
//////////////////Procedure bangers//////////////////////////////////////
        ...
        for(int f=0; f<127; f++)
        {
            ...
            {
                if(bangers_type[f][bgtest]!=0)
                {
                    there_is_someone_in_banger=1;
                    break;
                }
            }
    ...
    if(specify_who_to_save_PDF[8]==1)
    {
        ...
        for(int i=0; i<128; i++)
        {
            ...
            for(int tr=0; tr<24; tr++)
            {
                for(int c=1; c<513; c++)
                {
                    if(TrackContains[i][tr][c]!=0)
                    {
                        there_is_someone_in_chaser=1;
                        break;
                    }
                }
            }
...
//test de savoir si circuits affectes
            for(int col=0; col<4; col++)
            {
                for(int circ=1; circ<513; circ++)
                {
                    ...
                    {
                        there_is_someone_in_color_preset=1;
                        break;
                    }
                }
            }
...
///////////////////////////////////////////////////////////////////////////////
    if(specify_who_to_save_PDF[10]==1)
    {
        ...
        if (flip_image==1)
        {
            strcat(header_export,"Flip ON / ");
        }
        else
        {
            strcat(header_export,"Flip OFF / ");
        }
        if (threshold_on==1)
        {
            sprintf(little_header,"Threshold ON : %d / ",threshold_level);
        }
        else
        {
            sprintf(little_header,"Threshold OFF / ");
        }
        strcat(header_export,little_header);
        if (erode_mode==1)
        {
            sprintf(little_header,"Erode ON : %d / ",erode_level);
        }
        else
        {
            sprintf(little_header,"Erode OFF / ");
        }
        strcat(header_export,little_header);
        if (blur_on==1)
        {
            strcat(header_export,"Blur ON / ");
        }
        else
        {
            strcat(header_export,"Blur OFF / ");
        }
        draw_info(page, 50, debut_lignes -(position_ligne1 +(cmptline_pdf*12)),header_export);
        cmptline_pdf++;
        ...
        {
            sprintf(little_header,"SNAP ON , div factor: %d ",div_facteur);
        }
        else
        {
            sprintf(little_header,"SNAP OFF ( normal threshold capture)");
        }
        strcat(header_export,little_header);
        draw_info(page, 50, debut_lignes -(position_ligne1 +(cmptline_pdf*12)),header_export);
        cmptline_pdf++;
...
//test de l affectation d un circuit à la ROI
                ...
                for (int prp=0; prp<12; prp++)
                {
                    for(int circ=1; circ<512; circ++)
                    {
                        ...
                        {
                            there_is_a_roi_preset_affected=1;
                            break;
                        }
                    }
                }
...
///////////////////////////////////////////////////////////////////////////////
    ...
    if(specify_who_to_save_PDF[23]==1)
    {
        if(cmptline_pdf>20)
        {
            saut_de_page();   //pour fenetre de tracage
        }
