...
* \file MAIN.cpp
* \brief {main loop, can be compiled with Linux & g++}
...
#include <audiere.h>
#include <jpgalleg.h>
#include <string.h>         // std::string debug chaines caracteres
...
#include "patch_splines_2.h"//spline pour curves
bool curve_node::index_writing=0;
float curve_node::spline_level=0.0; //report en float
int curve_node::selected=0;
int curve_node::index_spline_level=0;//pixels de l editeur
bool curve_node::index_enable_editing=0;
int curve_node::diam=10;//diametre de la poignee pour saisie du curve_node
int curve_node::node_count=0;
/**
 * Il faudra lier avec le fichier patch_splines_2.o, qui est maintenant
 * indépendant, sinon ça ne marchera pas.
 **/
...
if(index_is_saving==0 && init_done==1 && curve_node::index_writing_curve==0 && index_quit==0)
{
for (int i=0;i<9;i++)
{
...
int main_actions_on_screen()
{
      ...
      if(curve_node::index_writing_curve==0){Boxes();}
...
if(old_ticks_arduino!=ticks_arduino && index_is_saving==0 && init_done==1 && curve_node::index_writing_curve==0 && arduino_device_0_is_ignited==1 && index_quit==0)//procedures de communication
{
    arduino_read();
