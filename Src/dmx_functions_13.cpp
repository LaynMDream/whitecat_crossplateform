...
//sab 02/03/2014 sprintf(string_time_left_is,"");
            ...
            {
                index_send_midi_out[491]=1;   //x1
            }
            if(midi_send_out[492]==1)
            {
                index_send_midi_out[492]=1;   //x2
            }
//auto go
...
//down only once
            else if(lfo_mode_is[cmptfader]==2 && actual_time>(start_time_for_delays[cmptfader]+time_delay_out[cmptfader]))
            {
                ...
                if(faders_in_float[cmptfader]<0)
                {
                    faders_in_float[cmptfader]=0;
                }
                if(FaderIsFlash[cmptfader]==0)//tous calculs ok si pas de flash
                {
                    Fader[cmptfader]=int(faders_in_float[cmptfader]);
                    ...
                    if(lfo_do_next_step[cmptfader][1]==1)//down dock
                    {
                        for (int tt=0; tt<core_user_define_nb_docks; tt++)
                        {
                            if ( DockIsSelected[cmptfader][tt]==1)
                            {
                                ...
                                if(tt<core_user_define_nb_docks-1)
                                {
                                    DockIsSelected[cmptfader][tt+1]=1;
                                    break;
                                }
                                else if(tt==core_user_define_nb_docks-1)
                                {
                                    DockIsSelected[cmptfader][0]=1;
                                    break;
                                }
                            }
                        }
                    ...
                    else if(lfo_do_next_step[cmptfader][0]==1)//up dock
                    {
                        for (int tt=0; tt<core_user_define_nb_docks; tt++)
                        {
                            if ( DockIsSelected[cmptfader][tt]==1)
                            {
                                ...
                                {
                                    DockIsSelected[cmptfader][core_user_define_nb_docks-1]=1;
                                    break;
                                }
                            }
                        }
...
//changement du niveau de lock, si on l augmente au dessus de son niveau de lock stocké
                if(Fader[cmptfader]>StateOfFaderBeforeLock[cmptfader] && FaderIsFlash[cmptfader]==0)
                {
...
//check channel override la sortie
        ...
        {
            if(patch_overide[i]==1)
            {
                DmxBlock[i]=dimmer_check_level;   //75%
            }
        }
...
int load_dmx_conf()
{
    ...
    if (strcmp (motcleinterfaceis,"ART-NET")==0)
    {
        myDMXinterfaceis=1;
    }
    else if (strcmp (motcleinterfaceis,"ENTTEC-OPEN-DMX")==0)
    {
        myDMXinterfaceis=2;
    }
    else if (strcmp (motcleinterfaceis,"ENTTEC-PRO")==0)
    {
        myDMXinterfaceis=3;
    }
    else if (strcmp (motcleinterfaceis,"SUNLITE")==0)
    {
        myDMXinterfaceis=4;
    }
...
int Save_my_dmx_conf()
{
    ...
    if( myDMXinterfaceis==1)
    {
        sprintf(motcleinterface,"ART-NET");
    }
    else if( myDMXinterfaceis==2)
    {
        sprintf(motcleinterface,"ENTTEC-OPEN-DMX");
    }
    else if( myDMXinterfaceis==3)
    {
        sprintf(motcleinterface,"ENTTEC-PRO");
    }
    else if( myDMXinterfaceis==4)
    {
        sprintf(motcleinterface,"SUNLITE");
    }
    FILE *fp;
