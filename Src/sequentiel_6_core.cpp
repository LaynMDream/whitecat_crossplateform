...
* \brief {Core fonctions for the cuelist}
...
int do_call_time_and_rec_f1_f2_f3(int mem_is)
{
    ...
    else if( index_do_dock==1 && index_affect_time==0 )
    {
        for (int dc=1; dc<514; dc++)
        {
            if(call_stage==1)
            {
                Memoires[position_onstage][dc]=0;
            }
            if(index_blind==0)
            {
    ...
    else if( index_do_dock==0 && index_do_modify==0 && index_do_report==1 && index_affect_time==0 )
    {
        if(index_blind==0)
        {
            for (int dc=1; dc<514; dc++)
            {
                if(bufferFaders[dc]>bufferSequenciel[dc])
                {
                    Memoires[mem_is][dc]=bufferFaders[dc];
                }
                else
                {
                    Memoires[mem_is][dc]=bufferSequenciel[dc];
...
//midi report
            ...
            else
            {
                ...
                {
                    if(((255.0-ratio_X1X2_together)/255)==1.0)
                    {
                        niveauX2=(255-niveauX1);
                    }
                    else
                    {
                        niveauX2=remapX2[(255-niveauX1)];
                    }
                    if(niveauX2<0)
                    {
                        niveauX2=0;
...
//midi report
            ...
            else
            {
                ...
                if(index_x1_x2_together==1)
                {
                    if(((255.0-ratio_X1X2_together)/255)==1.0)
                    {
                        niveauX1=(255-niveauX2);
                    }
                    else
                    {
                        niveauX1=255-remapX1[niveauX2];
                    }
                    if(niveauX1<0)
                    {
                        niveauX1=0;
...
//midi out des x1 et x2
    ...
    if(mouse_x> x_seq+655-10 && mouse_x<  x_seq+655+10 && mouse_y>y_seq+340-10 && mouse_y<y_seq+340+10)
    {
        ...
        if(midi_send_out[491]==0)
        {
            midi_send_out[491]=1;
        }
        else if(midi_send_out[491]==1)
        {
            midi_send_out[491]=0;
        }
...
//MIDI OUT DU SPEED
    ...
    {
        if(midi_send_out[493]==0)
        {
            midi_send_out[493]=1;
        }
        else if(midi_send_out[493]==1)
        {
            midi_send_out[493]=0;
        }
        mouseClicLeft.SetProcessed();
...
///alink
    ...
    {
        if(index_link_is_on==0)
        {
            index_link_is_on=1;
        }
        else if(index_link_is_on==1)
        {
            index_link_is_on=0;
        }
        mouseClicLeft.SetProcessed();
...
//UP DOWN griders number selected
        ...
        if( mouse_x>xseq+910-12 && mouse_x<xseq+910+12 && mouse_y>yseq+40-12 && mouse_y<yseq+40+12)
        {
            ...
            if( mouseClicLeft.isToBeProcessed())
            {
                ...
                {
                    if(index_grider_selected[0]<127)
                    {
                        index_grider_selected[0]++;
                    }
                }
        ...
        if(mouse_x>xseq+690 && mouse_x<xseq+690+105 && mouse_y>yseq+295 && mouse_y<yseq+295+15 && mouseClicLeft.isToBeProcessed())
        {
            if( Midi_Faders_Affectation_Type!=0)
            {
