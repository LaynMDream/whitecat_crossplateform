...
int Set_Filtering_Image()
{
    ...
    {
        hauteur_cadre_filter_image=130;
    }
...
//on off des functions
    ...
    if(flip_image==1)
    {
        OnFlip.Draw(CouleurFader);
    }
    if(threshold_on==1)
    {
        OnThreshold.Draw(CouleurFader);
    }
    if(erode_mode==1)
    {
        ModeErode.Draw(CouleurFader);
    }
    if(blur_on==1)
    {
        OnOffBlur.Draw(CouleurFader);
    }
...
//background window
    ...
    {
        frame_video_x=video_size_x;
        frame_video_y=video_size_y;
    }
    else
    {
        frame_video_x=352;
...
//AFFECT MODE POUR DOCKS
    ...
    if (index_affect_video_tracking_to_dock==1)
    {
        AffectVideoTo.Draw(CouleurFader);
    }
