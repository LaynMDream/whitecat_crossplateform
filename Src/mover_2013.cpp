...
int inversion_of_axes()
{
    if(invX_on==1)
    {
        ...
        if(tracker_mode==1)
        {
            buffer_moving_head[xadress+1]=255-mover_params[0][1];
        }
    }
    else if(invX_on==0)
    {
        ...
        if(tracker_mode==1)
        {
            buffer_moving_head[xadress+1]=mover_params[0][1];
        }
    }
    if(invY_on==1)
    {
        ...
        if(tracker_mode==1)
        {
            buffer_moving_head[yadress+1]=255-mover_params[1][1];
        }
    }
    else if(invY_on==0)
    {
        ...
        if(tracker_mode==1)
        {
            buffer_moving_head[yadress+1]=mover_params[1][1];
        }
    }
    return(0);
}
...
int Print_Points(int move_selected)
{
    ...
    for(int dms=1; dms<dock_moves_contains_steps[move_selected]+1; dms++)
    {
        ...
        if(dms==Moves_Inpoint[move_selected])
        {
            ShowInPoint.Draw(CouleurLock);
        }
...
//Depassement des ticks et fin crossfade
        if(move_current_time>move_end_value)
        {
            ...
            if(tracker_mode==0)//not spline
            {
                ...
                if (mover_params[1][0]<0)
                {
                    mover_params[1][0]=0;   //after init assume that it stays in the bounds [0;255]
                }
                else if (mover_params[1][0] >255)
                {
                    mover_params[1][0]=255;
