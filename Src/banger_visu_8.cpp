...
int alarm_window()
{
    ...
    {
        windowAlarm.DrawOutline(CouleurLigne);
    }
    char title_alarm[32];
...
//BANG DO IT
    ...
    if(do_light_bang_it==1)
    {
        BangerIt.Draw(CouleurFader);
        do_light_bang_it=0;
    }
    ...
    for (int lp=0; lp<6; lp++) //6 ev en bang
    {
        ...
        if(do_light_bang_solo[lp]==1)
        {
            Bangsolo.Draw(CouleurFader);
            do_light_bang_solo[lp]=0;
        }
