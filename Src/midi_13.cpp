...
//on prend que si en lock et maitre, ou alors sans lock
        if((FaderLocked[control]==1 && LockFader_is_FullLevel[control]==1)|| (FaderLocked[control]==0 ))
        {
            ...
            {
                Fader[control]=255;   //pour mettre à full
            }
...
//draw
    ...
    if(control==1652)//draw mode
    {
        ...
        if(draw_mode[draw_preset_selected]>4)
        {
    ...
    for (int i=1; i<3; i++)
    {
        ...
        if(mouse_x>cfg_midiX+20 && mouse_x<cfg_midiX+20+60 && mouse_y>cfg_midiY+60+(i*30) && mouse_y<cfg_midiY+60+(i*30)+15  && window_focus_id==920)
        {
            ...
            if(mouseClicLeft.isDown() &&  mouseClicLeft.isToBeProcessed() )
            {
                ...
                {
                    Midi_Faders_Affectation_Type=i;
                    do_affectation_on_midi_affect_itself=1;
                }
                mouseClicLeft.SetProcessed();
            }
        }
...
//Fake miditype
    ...
    if(fakemiditype==1)
    {
        petitchiffre.Print("Key On",cfg_midiX+725,cfg_midiY+210);
    }
    else if(fakemiditype==2)
    {
        petitchiffre.Print("Key Off",cfg_midiX+725,cfg_midiY+210);
    }
    else if(fakemiditype==4)
    {
        petitchiffre.Print("Ctrl Change",cfg_midiX+725,cfg_midiY+210);
    }
    ...
    for (int i=1; i<3; i++)
    {
        ...
        if(mouse_x>cfg_midiX+560 && mouse_x<cfg_midiX+560+60 && mouse_y>cfg_midiY+70+(i*30) && mouse_y<cfg_midiY+70+(i*30)+15  && window_focus_id==920)
        {
            ...
            if(mouseClicLeft.isDown() &&  mouseClicLeft.isToBeProcessed() )
            {
                ...
                {
                    Midi_Faders_Affectation_Mode=i;
                }
                mouseClicLeft.SetProcessed();
            }
        }
