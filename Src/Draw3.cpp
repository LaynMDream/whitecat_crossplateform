...
// draw_level_to_do[pr]/BPS_RATE car dans la boucle data
        ...
        if(draw_brush_type[pr]==0)//Point
        {
            ...
            case 0://draw
                ...
                if(draw_preset_levels[pr][coord]>1.0)
                {
                    draw_preset_levels[pr][coord]=1.0;
                }
                break;
            case 1://erase
                draw_preset_levels[pr][coord]-=draw_level_to_do[pr]/rate ;
                if(draw_preset_levels[pr][coord]<0.0)
                {
                    draw_preset_levels[pr][coord]=0.0;
                }
                break;
            case 2://solo
                for(int i=0; i<500; i++)
...
int Draw_Window(int xw,int yw)
{
    ...
    {
        DrawBckgd.DrawOutline(CouleurLigne);
    }
...
//SELECTION PRESETS
    ...
    for(int pr=0; pr<6; pr++)
    {
        ...
        if(draw_preset_selected==pr)
        {
            DrawPst.Draw(CouleurFader);
        }
...
//fin presets
    ...
    if(index_affect_draw_to_dock==1)
    {
...
//recup anim grid player gridplayer
    ...
    {
        if(mouse_x>xw+455 && mouse_x<xw+455+30)
        {
            NumGPL.DrawOutline(CouleurLigne);
        }
        else if(mouse_x>xw+495 && mouse_x<xw+495+35)
        {
            OffsetGPL.DrawOutline(CouleurLigne);
        }
    }
...
int do_logical_Draw_TrackingAera(int xw,int yw)
{
    if(window_focus_id==W_DRAW && mouse_x>=xw && mouse_x<=xw+draw_tracking_aera_size_x && mouse_y>=yw && mouse_y<=yw+draw_tracking_aera_size_y && mouseClicLeft.isDown())
    {
        if( dragging_draw==0)
        {
            snap_state_of_draw_grid(draw_preset_selected);
            dragging_draw=1;
        }
...
//affectation mode
    if(window_focus_id==W_DRAW  && mouseClicLeft.isDown() && mouseClicLeft.isToBeProcessed() && mouse_x> xw+400 && mouse_x<xw+400+110 && mouse_y>yw+75 && mouse_y<yw+75+20 )
    {
        ...
        if(draw_channel_recording_mode>3)
        {
            draw_channel_recording_mode=0;
        }
        mouseClicLeft.SetProcessed();
    }
////AFFECT TO DOCK
